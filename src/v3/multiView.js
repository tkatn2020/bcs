...
const VIEWS = [
  ...
  // 아바타 '왼쪽 뒤 사선'(+x, −z)에서 전방 콘 영역을 바라보는 시점(사용자
  // 확정): 콘 축을 따라 비스듬히 보므로 원·중·근 시야콘의 원형 밑면이 크고
  // 둥근 타원으로 온전히 보인다(측면 수직 시점은 밑면이 선으로 눌려 실패).
  // 카메라를 충분히 뒤로(−z 0.25) 빼 왼쪽 뒤통수·귀가 앞을 가려 코가 안
  // 보이고, 아래로 조준해 근용(황금) 밑면이 안 잘린다. 청록(중)·황금(근)
  // 밑면이 중앙~하단 타원으로 전개.
  { key: 'side',  label: '측면 · 시야 분출',  pos: [0.44, 0.05, -0.25],    tgt: [-0.03, -0.09, 0.46], cones: true, face: true },
];
