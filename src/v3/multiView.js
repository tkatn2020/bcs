...
const VIEWS = [
  ...
  // 줌아웃+콘 영역 조준 — 원·중·근 시야콘의 '원형 밑면(시야 풋프린트)'이
  // 타원으로 온전히 보이는 구도(사용자 핵심 요구): 머리는 우측, 근(황금)·
  // 중(청록) 콘의 밑면이 좌측에 펼쳐진다. (원거리 콘은 z1.6까지 뻗어 밑면은
  // 화면 밖 — 첨부도 동일하게 원거리는 옅게만.)
  { key: 'side',  label: '측면 · 시야 분출',  pos: [0.84, 0.05, 0.22],     tgt: [0, -0.05, 0.44], cones: true,  face: true },
];
